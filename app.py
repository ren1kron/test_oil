...
editor, results_tab, comparison, optimization, risks_tab, assumptions_tab = st.tabs(["План", "Результаты", "Сравнение", "Оптимизация", "Риски", "Данные и допущения"])
pending_edits = False
with editor:
...
with results_tab:
    ...
    if result and (pending_edits or st.session_state.get("result_signature") != signature(plan, case, scenario)):
        ...
        st.plotly_chart(px.bar(costs, x="Год", y=["Закупки, млн", "Резерв мощности, млн", "Хранение, млн", "Постоянный OPEX, млн", "CAPEX, млн"], title="Структура расходов"), width="stretch")
...
with risks_tab:
    st.caption("Исследовательский сценарий применяется отдельно к BASE. Вероятности не назначаются. Стоимость мер снижения риска не включена без отдельного обоснования.")
    ...
    risk = Risk(risk_id="TEAM_SUPPLY", event="Задержка / недопоставка выбранного источника", source_id=source, start_year=years[0], end_year=years[1], delivery_share=share, delay_days=delay, price_multiplier=price, owner=owner, mitigation=mitigation, residual_delay_days=residual_delay, residual_delivery_share=residual_share)
    risk_sig = signature(plan, case, risk.model_dump_json())
    if st.button("Рассчитать риск", disabled=pending_edits):
        try:
...
- Резерв — физический. Emergency не заменяет запас автоматически. Любая плановая закупка Emergency считается базовым использованием.
