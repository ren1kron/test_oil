...
Рекомендуется стратегия diversified с заранее сохраненной процедурой ответа на обязательный стресс.
...
Так же был расcчитан unrestricted план - свободный запуск оптимизатора без пренудительного выбора инвестиционной архитектуры. Оптимизатор к практически идентичному lunar плану, поэтому unrestricted не выделяется как самостоятельная альтернатива и используется только как технический benchmark.
